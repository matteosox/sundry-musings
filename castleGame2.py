...
solutions = pd.DataFrame({'entry': [tuple(path[-1][0]) for path in pathsRd1],
...
def nextScorer(scorer, paths=None):
    ...
    totalPaths = \
        pd.DataFrame({'entry': [tup[0] for path in paths for tup in path],
    ...
    field = np.array([entry for entry in
                      winPaths.entry.sample(n=scorer.field.shape[0],
                                            replace=True)])
...
def constructScorer(scorerRd1, nEntries, dist, trollThresh=8, pathsRd1=None):
    ...
    [nextScorers.append(nextScorer(nextScorers[-1]))
        for dummy in range(len(dist) - 3)]

    trollSample = pd.DataFrame(trollsRd1).sample(n=nSamples[0],
                                                 replace=True).values
    rd1Sample = pd.DataFrame(compsRd1).sample(n=nSamples[1],
                                              replace=True).values
...
for dummy in range(runs):
    ...
    dist = np.hstack((trollDist, otherDists /
                      otherDists.sum() * (1 - trollDist)))
    rd2Pred2Scorer, scorers = constructScorer(rd1Scorer, 1000, dist,
                                              pathsRd1=pathsRd1)
...
uniqueEntriesRd2Act = solutionsRd2Act.groupby('entry').mean(). \
    sort_values('margin', ascending=False)
...
uniqueEntriesRd2Pred = solutionsRd2Pred.groupby('entry').mean(). \
    sort_values('margin', ascending=False)
...
rd2PredMargins = np.array([rd2ActScorer.margin(entry)
                           for entry in uniquesRd2Pred['entry']])
...
ax.fill_between(x, conf[0].flatten(), conf[1].flatten(), alpha=0.3,
