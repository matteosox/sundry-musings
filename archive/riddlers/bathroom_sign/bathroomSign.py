...
pSignOcc = pSignOccGivenVac * (1 - pOcc) + (
    pSignOccGivenVac * pOcc * p1 + 1 * pOcc * p2 + 1 * pOcc * p3
)
pOccGivenSignOcc = pSignOccGivenOcc * pOcc / pSignOcc
print(
    "The probability the sign says occupied given that the room is occupied "
...
print(
    "The probability the sign says vacant given that the room is vacant "
...
vMin = (
    np.round(min(pOO[np.isfinite(pOO)].min(), pVV[np.isfinite(pVV)].min()) * 100) / 100
)
vMax = (
    np.round(max(pOO[np.isfinite(pOO)].max(), pVV[np.isfinite(pVV)].max()) * 100) / 100
)
