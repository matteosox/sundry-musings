...
fig = plotHist(
    fieldRd1, scores=rd1Scorer.margins, scoreLabel="Win Margin (%)", fieldName=""
)
...
fig = plotParCoords(fieldRd1, scores=rd1Scorer.percentiles, scoreLabel="Percentile (%)")
...
ax.plot(entries[:, 0], entries[:, 1], "o", color="#4b78ca", ms=10, label="entries")
...
ax.plot(entries[:, 0], entries[:, 1], "o", color="#4b78ca", ms=8, label="entries")
ax.fill_between([0, 3], [3, 0], color="w", alpha=0.3, lw=0, label="x + y + z = 3")
...
ax.plot_trisurf([3, 0, 0], [0, 3, 0], [0, 0, 3], label="w = 3", shade=False, alpha=0.3)
ax.plot_trisurf([2, 0, 0], [0, 2, 0], [0, 0, 2], label="w = 2", shade=False, alpha=0.3)
ax.plot_trisurf([1, 0, 0], [0, 1, 0], [0, 0, 1], label="w = 1", shade=False, alpha=0.3)
...
entriesW = normEntries(
    np.random.random((nEntries, nCastles)) * np.tile(rd1Scorer.weights, (nEntries, 1))
)
entriesRd1 = normEntries(
    fieldRd1[
        np.random.randint(0, fieldRd1.shape[0], size=(nEntries, fieldRd1.shape[1])),
        np.tile(np.arange(nCastles), (nEntries, 1)),
    ].astype(float)
)
entriesLHS = normEntries(lhs(nCastles, nEntries))
entriesMulti = np.random.multinomial(
    nTroops, [1 / nCastles] * nCastles, size=nEntries
).astype(np.uint8)
entriesWMulti = np.random.multinomial(
    nTroops, rd1Scorer.weights / rd1Scorer.weights.sum(), size=nEntries
).astype(np.uint8)
entriesRd1AvMulti = np.random.multinomial(
    nTroops, fieldRd1.mean(0) / fieldRd1.mean(0).sum(), size=nEntries
).astype(np.uint8)
entriesRd1MedMulti = np.random.multinomial(
    nTroops,
    np.median(fieldRd1, axis=0) / np.median(fieldRd1, axis=0).sum(),
    size=nEntries,
).astype(np.uint8)
...
fig = plotHist(
    fieldRd2Pred,
    scores=rd2PredScorer.margins,
    scoreLabel="Win Margin (%)",
    fieldName="Predicted Round 2",
)
fig.savefig("Castle Histograms")

fig = plotParCoords(
    fieldRd2Pred, scores=rd2PredScorer.percentiles, scoreLabel="Percentile (%)"
)
