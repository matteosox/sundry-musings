...
raw = pd.read_csv(
    "./Data_Extract_From_Poverty_and_Equity/"
...
countries = [
    loc for loc in locs if (loc not in regions) and (loc not in incomeBrackets)
]

countryCodes = [
    data.loc[data["Country"] == country, "Country Code"].values[0]
    for country in countries
]
regionDict = {
    country: regionData.loc[regionData["alpha-3"] == code, "region"].values[0]
    ...
    if (isFin.sum() >= 2) and not isFin.all():
        ...
        for yr in row[~isFin].index:
            if (yr > minYr) and (yr < maxYr):
                data.loc[ind, yr] = np.interp(
                    yr, row[isFin].index.values.astype(int), row[isFin].values
                )
...
for country in countries:
    pop = (
        data.loc[
            (data["Country"] == country) & (data["Series Code"] == "SP.POP.TOTL"), :
        ]
        .iloc[:, 4:]
        .values
    )
    ...
    gini = (
        data.loc[
            (data["Country"] == country) & (data["Series Code"] == "SI.POV.GINI"), :
        ]
        .iloc[:, 4:]
        .values
    )
    ...
    gdp = (
        data.loc[
            (data["Country"] == country) & (data["Series Code"] == "SI.SPR.PCAP"), :
        ]
        .iloc[:, 4:]
        .values
    )
    ...
    if np.isfinite([pop, gini, gdp]).all():
        ...
        ax.plot(gdp, gini, "o", ms=ms, color=colorDict[region], alpha=0.7, mew=0)
        if pop > 50e6:
...
for region, color in colorDict.items():
    ax.plot(-100, -100, "o", color=color, label=region, ms=10, alpha=0.7, mew=0)
...
sub = data.loc[isRegion & (data["Series Code"] == "SI.POV.NOP1"), :]
x = sub.iloc[:-1, 20:-2].columns.values.astype(int)
y = sub.iloc[:-1, 20:-2].values
y = np.concatenate((y, sub.iloc[-1, 20:-2].values - y.sum(0, keepdims=True))).astype(
    float
)
...
sub = data.loc[isGroup & (data["Series Code"] == "SI.POV.NOP1"), :]
x = sub.iloc[:-1, 20:-2].columns.values.astype(int)
y = sub.iloc[:-1, 20:-2].values
y = np.concatenate((y, sub.iloc[-1, 20:-2].values - y.sum(0, keepdims=True))).astype(
    float
...
for region in shortRegions:
    ...
    dst110s = (
        data.loc[isRegion & (data["Series Code"] == "SI.DST.FRST.10"), :]
        .iloc[:, 4:]
        .values
    )
    dst1010s = (
        data.loc[isRegion & (data["Series Code"] == "SI.DST.10TH.10"), :]
        .iloc[:, 4:]
        .values
    )
    pops = (
        data.loc[isRegion & (data["Series Code"] == "SP.POP.TOTL"), :]
        .iloc[:, 4:]
        .values
    )
...
for region, color in colorDict.items():
    ax2.plot(-100, -100, "o", color=color, label=region, ms=10, mew=0)
...
for country, color in zip(interest, colors):
    ...
    pov = (
        data.loc[
            (data["Country"] == country) & (data["Series Code"] == "SI.POV.DDAY"), :
        ]
        .iloc[:, 4:]
        .values
    )
